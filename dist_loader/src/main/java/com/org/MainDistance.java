...
public class MainDistance {
    public static void main(String[] args) {
        QueryLogReader reader = new QueryLogReader("/data/DBpedia2016_0_1_10_v3_weight_loss/train_sampled.tsv");
        try {
            reader.read();
        } catch (IOException e) {
            e.printStackTrace();
        ...
        for (int j = 0; j < queryIds.size(); j++) {
            for (int v = j; v < queryIds.size(); v++) {
                double val = distLoader.get(queryIds.get(j), queryIds.get(v));
                if (val == -1) {
                    missingPairs++;
                } else {
                    total += val;
