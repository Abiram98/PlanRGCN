...
def entity_stat_extraction(parser:configparser.ConfigParser):
    ...
    if (not os.path.isfile(entity_path)) or (not os.path.isfile(entity_freq_dict_path)) :
        ...
        freq_dict = ent_featurizer.get_freq_all_entities(entities,save_path=entity_freq_dict_path)
    else:
...
if __name__ == "__main__":
    ...
    match( args.cmd): 
        ...
        case 'plot':
            entities, freq_dict = load_feature_data(parser)
            plot_entity_stat(entities, freq_dict, save=parser['Results']['fig_dir']+'entity_dist.png')
            pass
        case 'binnify':
