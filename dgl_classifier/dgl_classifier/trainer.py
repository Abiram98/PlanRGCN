...
import torch
from graph_construction.tps_graph import (
    create_dummy_dgl_graph,
    tps_graph,
    get_tp_graph_class,
)
...
def extract_data(
    ...
    if is_run_clust:
        featurizer = BaseFeaturizer(
            ...
            path_pred_clust={
                "save_path": "/work/data/confs/14June/pred_clust.json",
                "load_path": None,
            },
        ...
        featurizer = BaseFeaturizer(
            ...
            path_pred_clust={
                "save_path": None,
                "load_path": "/work/data/confs/14June/pred_clust.json",
            },
            verbose=clust_verbose,
        )
    ...
    train_dgl, train_gt = [], []
    for x in train_bgps:
        dgl_graph = tps_graph_const(
            x, featurizer=featurizer, TP_graph_type=TP_graph_type
        )
        if dgl_graph != None:
...
def predict2samples(
    ...
    featurizer = BaseFeaturizer(
        ...
        save_pred_graph_png=None,
        community_no=community_no,
        path_pred_clust={"save_path": None, "load_path": clust_load_path},
        verbose=clust_verbose,
    )

    train_bgps = load_BGPS_from_json(train_path, TP_graph_type=TP_graph_type)
    train_data = predictBgp2samples(
        train_bgps,
        train_path,
        featurizer,
        model,
        ...
        add_thres=add_thres,
        TP_graph_type=TP_graph_type,
    )

    val_bgps = load_BGPS_from_json(val_path, TP_graph_type=TP_graph_type)
    val_data = predictBgp2samples(
        val_bgps,
        val_path,
        featurizer,
        model,
...
if __name__ == "__main__":
    ...
    exit()
    train_dataloader, val_dataloader, test_dataloader = extract_data(
        train_file, val_file, test_file, community_no=30, batch_size=50, verbose=True
    )
