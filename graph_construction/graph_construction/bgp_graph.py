...
class BGPGraph:
    ...
    def variable_node_check(self, node: Node):
        if node.type == "VAR":
            ...
            if not node.node_label in self.vars.keys():
                ...
                node2.node_label = node2.node_label + str(
                    len(self.vars[node.node_label])
                )
    ...
    def get_node_representation(
        self,
    ):  # pred_bins, topk, pred_feat_sub_obj_no=True,use_ent_feat=False, ent_bins = None
        ...
        return np.stack(
            [
                x.get_features(
                    pred_bins=pred_bins,
                    pred_topk=topk,
                    pred_feat_sub_obj_no=pred_feat_sub_obj_no,
                    use_ent_feat=use_ent_feat,
                    ent_bins=ent_bins,
                )
                for x in self.nodes
            ]
