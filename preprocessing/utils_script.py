...
from preprocessing.utils import (
    convert_leaf_to_json,
    get_predicates_from_path,
    ground_truth_distibution,
    load_BGPS_from_json,
)
...
def data_stats():
    ...
    print(
        f"Pred jarcard between train and validation: {jaccard(train_preds,val_preds)}"
    )
    print(f"Pred jarcard between train and test: {jaccard(train_preds,test_preds)}")


def get_bin_topk_dict_rdf(
    PRED_FEATURIZER, feat_generation_path, bin_no, topk, NODE, parser
):
    pred_feature_rizer: Predicate_Featurizer_Sub_Obj = (
        PRED_FEATURIZER.prepare_pred_featues_for_bgp(
            feat_generation_path, bins=bin_no, topk=topk, obj_type=PRED_FEATURIZER
        )
    )
...
def get_bin_topk_dict_ql(
    PRED_FEATURIZER, feat_generation_path, bin_no, topk, NODE, parser
):
    pred_feature_rizer: ql_pred_featurizer = (
        PRED_FEATURIZER.prepare_pred_featues_for_bgp(
            feat_generation_path, bins=bin_no, topk=topk, obj_type=PRED_FEATURIZER
        )
    )
...
def plot_bucket_stat(
    bucket_dct, bin_no, path="/work/data/confs/May2/pred_charts", color="green"
) -> int:
    dct = {"buckets": [], "Count": []}
    ...
    plt.clf()
    plt.barh(
        [f"bucket_{x}" for x in list(df["buckets"])],
        list(df["Count"]),
        0.75,
        color=color,
    )
...
def pred_bucket_analysis_rdf():
    ...
    for bin_no in range(20, 50):
        pred_buckets, _ = get_bin_topk_dict_rdf(
            PRED_FEATURIZER, feat_generation_path, bin_no, topk, NODE, parser
        )
        # print("Bucket stats")
    ...
    print(
        f"Binning with bucket size {max_bin_no} has most used bucket [{max_bucket_size}]"
    )

    exit()
    pred_buckets, pred_topk = get_bin_topk_dict_rdf(
        PRED_FEATURIZER, feat_generation_path, bin_no, topk, NODE, parser
    )
    print("topk stats")
...
def pred_bucket_analysis_querylog(path="/work/data/confs/May2/query_log_preds"):
    ...
    for bin_no in range(5, 50):
        pred_buckets, _ = get_bin_topk_dict_ql(
            PRED_FEATURIZER, feat_generation_path, bin_no, topk, NODE, parser
        )
        # print("Bucket stats")
        # for key in pred_buckets.keys():
...
def re_gt_analysis(bgps: dict, threshold):
    ...
    for k in bgps.keys():
        if (
            relative_error_v2(
                int(bgps[k]["bloom_runtime"]), int(bgps[k]["jena_runtime"])
            )
            > threshold
        ):
            ones += 1
...
def process_gt(path, output, gt_type="re", th=None):
    ...
    if gt_type == "std":
        ...
        print(
            f'Please provide a legal Ground Truth assignment type. "{gt_type}" is not legal.'
        )
        exit()
...
def gt_median_assignment(bgps, runtime_field="jena_runtime"):
    ...
    for k in bgps.keys():
        ...
        n_bgps[k] = temp_dct
    print(
        f"Distribution of GT: 1:{ones} [{ones/(ones+zeros)}] 0:{zeros} [{zeros/(ones+zeros)}]"
    )
    return n_bgps
...
def gt_re_assignemnt(bgps, threshold=0.1):
    ...
    for k in bgps.keys():
        ...
        if (
            ...
            zeros += 1
    print(
        f"GT distribution: zero {zeros} [{zeros/(zeros+ones)}], ones {ones} [{ones/(zeros+ones)}] with threshold {threshold}"
    )
    return n_bgps
...
if __name__ == "__main__":
    ...
    if args.task == "split":
        ...
        stratified_split_preds(
            input_path=args.input, output=args.output, equalise_gt=True
        )
