...
from classifier.batched.gcn import GNN, GNN_RT, GNN_w_Dense
import configparser
...
def run_undivided(train_loader:DataLoader, validation_loader:DataLoader, path_to_save ='/work/data/confs/newPredExtractionRun/', epoch=50, early_stop=10, layer_size=100):
    ...
    model = GNN_RT(train_loader.dataset[0].x.shape[1] , train_loader.dataset[0].x.shape[1]*2)
    model = model.float()
    ...
    for t in range(epoch):
        ...
        for sample_no, batch in enumerate(train_loader):
            ...
            re_batch = relative_error(batch.y, pred)
            print(f"Epoch: {t+1:4} {(sample_no+1):8} Batch loss: {c_train_loss:>7f} Batch RE: {re_batch}")
        ...
        with torch.no_grad():
            for no,sample in enumerate(validation_loader):
                ...
                re_batch_val = relative_error(sample.y,pred)
                val_re += re_batch_val
        val_loss= val_loss/len(val_loader)
        val_re= val_re/len(val_loader)
        train_loss = train_loss/len(train_loader)
        train_re = train_re/len(train_loader)
        ...
        print(f"Train Avg RE {t+1:4}: {train_re}\n")
        print(f"Val Avg Loss {t+1:4}: {val_loss:>8f}\n")
        print(f"Val Avg RE {t+1:4}:  {val_re}\n")
...
def get_debug_data_loader(parser):
    ...
    #train has to be first for scaling
    train_loader = DataLoader(get_graph_representation(data_file, node=NODE,  norm='min_max', train=True), batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(get_graph_representation(val_file, node=NODE, norm='min_max', train=False), batch_size=BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(get_graph_representation(test_file, node=NODE, norm='min_max', train=False), batch_size=BATCH_SIZE, shuffle=True)
