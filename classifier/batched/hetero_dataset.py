...
def get_graph_representation(parser, data_file) -> None:
...
def get_graph_for_single_sample(bgp_string=None, ground_truth=None, pred_feat=None, bin_no=None, topk=None, bgp_graph = None):
    if bgp_graph == None:
        bgp = BGP(bgp_string, ground_truth,predicate_stat=pred_feat)
        bgp_graph = BGPGraph(bgp)
        node_feat = torch.tensor( bgp_graph.get_node_representation(bin_no, topk, pred_feat_sub_obj_no=True, use_ent_feat=False), dtype=torch.float32)
